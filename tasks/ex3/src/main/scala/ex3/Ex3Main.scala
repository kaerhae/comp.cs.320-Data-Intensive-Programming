...
object Ex3Main extends App {
  ...
  .option("header", "true")
  .option("delimiter", ";")
  .csv("../ex3/data/sales_data_sample.csv")
  ...
  val best10DaysDF: DataFrame = retailerDataFrame.withColumn("sellingDay", retailerDataFrame("QUANTITYORDERED") * retailerDataFrame("PRICEEACH")).sort(col("sellingDay").desc).limit(10)

  best10DaysDF.foreach(x => println("Line: " + x))
  ...
  val salesDS: Dataset[Sales] = spark.createDataFrame(salesList).as[Sales]
  ...
  val multiSalesDS: Dataset[Sales] = spark.createDataFrame(multiSalesList).as[Sales]

  /* 
    /// AUTHORS COMMENT ////

    Absolutely awful chaining practise, but works still. Chaining does the following:
      1) Filter dataset by year 2019
      2) Transform to dataframe
      3) Creating column "year" by grouping
      4) With aggregating, sum up column "euros" and rename sum("euros") to "euros"
      5) For some reason sum() returns a bigint so, have to cast it to integer because case class Sales type is integer
      6) Transform back to dataset
      7) Return first (and only) element of data for the sake of returning type as Sales
  */
  ...
  val maximumMultiSales: Sales = multiSalesDS.filter(s => s.year == 2019).orderBy(col("euros").desc).head()
  ...
  val best10DaysDFStreaming = retailerStreamingDF.withColumn("sellingDay", retailerDataFrame("QUANTITYORDERED") * retailerDataFrame("PRICEEACH")).sort(col("sellingDay").desc).limit(10)
