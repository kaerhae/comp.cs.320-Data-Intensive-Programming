...
import org.apache.spark.sql.functions.{count, sum, min, max, asc, desc, udf, to_date, avg, explode}
...
object Ex4Main extends App {
  ...
  val dfSchema: StructType = new StructType(Array(
    new StructField("X", DoubleType, true),
    new StructField("label", DoubleType, true)
  ))
  val dataDF: DataFrame = spark.createDataFrame(dataRDD, dfSchema)
  ...
  val pipeline = transformationPipeline.fit(dataDF)
  val assembledTrainingDF: DataFrame = pipeline.transform(trainingDF)
  ...
  val assembledDataDF = pipeline.transform(dataDF)
  val allPredictions: DataFrame = lrModel.transform(assembledDataDF)
  allPredictions.show()
  ...
  // Task 5: Use the LinearRegressionModel to predict y for values [-0.5, 3.14, 7.5]
    ...
    val newValueRDD: RDD[Row] = spark.sparkContext.parallelize(Seq(Row(-0.5), Row(3.14), Row(7.5)))
    
    val newDataDF: DataFrame = spark.createDataFrame(newValueRDD, 
      new StructType(Array(
        new StructField("X", DoubleType, true)
      ))
    )

    val assembledNewDataDF = pipeline.transform(newDataDF)
    val predict: DataFrame = lrModel.transform(assembledNewDataDF)
    predict.show()
  ...
  val data: DataFrame = spark.read
    ...
    .load("./data/numbers.csv")

  val assembledCsvDataDF = pipeline.transform(data)

  
  val numberPredictionsDF: DataFrame = lrModel.transform(assembledCsvDataDF)
  ...
  /* numberPredictionsDF contains unsupported vector datatype 
    * Casting column 'features' to arraylike string with Scala 3.0 built-in vector_to_array and cast("string")*/
  val castedPredictions: DataFrame = numberPredictionsDF.withColumn("features", vector_to_array(numberPredictionsDF("features")).cast("string"))
 
  /* Generating timestamp as a string to variable ts */
  val ts: String = DateTimeFormatter.ofPattern("yyyyMMddHHmm").format(LocalDateTime.now)

  /* Formatting file name in 'resultYYYYMMDDHHMM' format */
  castedPredictions.write.csv(s"./results/result${ts}")
