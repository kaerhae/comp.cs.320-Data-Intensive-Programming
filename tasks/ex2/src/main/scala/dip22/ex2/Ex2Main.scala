...
object Ex2Main extends App {
  ...
  val weatherDataFrame: DataFrame = spark.read.option("inferSchema", "true").option("header", "true").csv("../ex2/data/rdu-weather-history.csv")
  ...
  val minTemp: Double = weatherDataFrame.agg(min("temperaturemin"), max("temperaturemin")).head.getDouble(0)
  val maxTemp: Double = weatherDataFrame.agg(min("temperaturemax"), max("temperaturemax")).head.getDouble(1)
  ...
  val weatherDataFrameWithYear: DataFrame = weatherDataFrame.withColumn("year", year(to_timestamp(weatherDataFrame("date"))))
  ...
  val aggregatedDF: DataFrame = weatherDataFrameWithYear.groupBy("year").agg(
    min("temperaturemin").as("temperaturemin"),
    max("temperaturemax").as("temperaturemax")
  ).orderBy("year")
  ...
  val task6DF: DataFrame = weatherDataFrameWithYear.groupBy("year").agg(
    count("year").as("amountOfRecords"),
    round(avg("avgwindspeed"), 2).as("averageWindSpeed")
  )
